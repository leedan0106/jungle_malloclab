...
static void *extend_heap(size_t words) 
{
    ...
    PUT(HDRP(NEXT_BLKP(bp)), PACK(0, 1)); /* New epilogue header.. epilogue header를 왜 또 추가해 주는 건지? Prologue header는 안해주는데.. 가용 블럭의 끝을 구분하기 위해서 새로운 가용 블록을 추가할 때 새로 만들어주는 건가? */
...
static void *coalesce(void *ptr) {
    ...
    if (prev_alloc && next_alloc) { /* Case 1: 앞 뒤 블록 모두 할당되어 있는 경우: 연결할 블록 없음 */
        ...
        // if (NEXT_BLKP(ptr) == last_find_ptr) { // 통합되서 앞으로 붙게 되는 블럭이 현재 last_find_ptr인 경우 이전 블록으로 업데이트
        //     last_find_ptr = ptr;   
        // }
        ...
        // if (ptr == last_find_ptr) { // 이전 블록과 합쳐질 현재 블록이 last_find_ptr인 경우 이전 블록으로 업데이트
        //     last_find_ptr = PREV_BLKP(ptr);
        // }
        ...
        // if (ptr == last_find_ptr || NEXT_BLKP(ptr) == last_find_ptr) {
        //     last_find_ptr = PREV_BLKP(ptr);
        // }
...
{      
    ...
    void *cur_ptr = last_find_ptr;

    while(GET_SIZE(HDRP(cur_ptr)) != 0) {
        if (GET_SIZE(HDRP(cur_ptr)) >= asize && !GET_ALLOC(HDRP(cur_ptr))) {
            // last_find_ptr = NEXT_BLKP(cur_ptr); // 현재 할당 받게 될 블록의 다음 블록을 저장.
            last_find_ptr = cur_ptr;
            return cur_ptr;
        }
        else {
